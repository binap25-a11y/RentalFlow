 ...
 * DESIGNED FOR RESILIENCE: Actions fail silently/gracefully so UI workflows are not blocked by unverified domains.
 */
    ...
    return await sendPropertyEmail({
      ...
      html: `
        <div style="font-family: sans-serif; padding: 30px; background-color: ${BRAND_BG}; border-radius: 24px; color: #1e293b;">
          ...
          <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:9002'}/landlord/maintenance" style="display: inline-block; background: ${BRAND_PRIMARY}; color: white; padding: 12px 24px; text-decoration: none; border-radius: 12px; font-weight: bold; margin-top: 10px;">Review Request</a>
        </div>
      `
    });
  } catch (e) {
    console.warn('Graceful Fallback: Landlord email trigger failed silently.');
      ...
      html: `
        <div style="font-family: sans-serif; padding: 40px; background-color: ${BRAND_BG}; border-radius: 32px; color: #1e293b; max-width: 600px; margin: auto;">
          ...
          <div style="background: white; padding: 25px; border-radius: 20px; margin: 30px 0; border: 1px solid #e2e8f0; text-align: center;">
            ...
            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:9002'}" style="display: inline-block; background: ${BRAND_PRIMARY}; color: white; padding: 14px 32px; text-decoration: none; border-radius: 14px; font-weight: bold; margin-top: 20px; box-shadow: 0 10px 15px -3px rgba(30, 58, 138, 0.2);">Access Resident Hub</a>
          </div>
      ...
      html: `
        <div style="font-family: sans-serif; padding: 40px; background-color: ${BRAND_BG}; border-radius: 32px; color: #1e293b; max-width: 600px; margin: auto;">
          ...
          <div style="text-align: center; margin-top: 30px;">
            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:9002'}/tenant/hub" style="display: inline-block; background: ${BRAND_PRIMARY}; color: white; padding: 14px 32px; text-decoration: none; border-radius: 14px; font-weight: bold;">View Tenancy Status</a>
          </div>
        </div>
      `
    });
      ...
      html: `
        <div style="font-family: sans-serif; padding: 40px; background-color: ${BRAND_BG}; border-radius: 32px; color: #1e293b; max-width: 600px; margin: auto;">
          <div style="text-align: center; margin-bottom: 30px;">
            ...
            <h2 style="color: ${BRAND_PRIMARY}; margin-top: 0;">Payment Verified</h2>
