 ...
 * Refined with professional standardized smaller hero actions.
 */
...
export default function LandingPage() {
  ...
  if (!mounted) {
    return <div className="fixed inset-0 bg-background z-[100] transition-opacity duration-300" />;
  }
  ...
  return (
    <div className={cn(
      ...
      <section className="relative pt-24 pb-40 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 flex flex-col items-center text-center space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <div className="flex flex-col items-center gap-10">
            <div className="flex flex-col items-center">
              <div className="relative h-24 w-24 rounded-[2rem] overflow-hidden shadow-2xl ring-1 ring-white/10 bg-card transition-transform hover:scale-105 duration-700">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Button size="lg" asChild className="h-14 px-12 rounded-2xl bg-accent hover:bg-accent/90 text-white text-base font-bold shadow-2xl transition-all hover:scale-[1.02] active:scale-95 border-none">
                <Link href="/auth">Sign In <ArrowRight className="w-5 h-5 ml-3" /></Link>
              </Button>
              <Button size="lg" variant="outline" className="h-14 px-10 rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 text-foreground font-bold text-base backdrop-blur-md cursor-pointer transition-all" onClick={scrollToFeatures}>
      ...
      <section id="features" className="py-40 bg-muted/20 relative">
        <div className="max-w-7xl mx-auto px-6 text-center space-y-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {[
              { title: "AI Maintenance Triage", desc: "Prioritize critical repairs and receive automated fix strategies the microsecond an issue is reported.", icon: Zap, color: "text-amber-500", bg: "bg-amber-500/10" },
              { title: "Financial Command", desc: "Unified rental ledgers with real-time collection status and automated professional visual records.", icon: Activity, color: "text-accent", bg: "bg-accent/10" },
              { title: "Resident Concierge", desc: "A dedicated AI assistant for residents, providing instant guidance on property protocols and guides.", icon: Bot, color: "text-emerald-500", bg: "bg-emerald-500/10" }
            ].map((f, i) => (
              <Card key={i} className="border-none shadow-sm rounded-[3rem] p-12 bg-card hover:shadow-2xl transition-all duration-500 group text-left ring-1 ring-white/5 hover:ring-white/10 hover:scale-[1.01]">
                <div className={cn("p-6 rounded-[2rem] w-fit mb-10 group-hover:scale-110 transition-transform duration-500", f.bg, f.color)}><f.icon className="w-10 h-10" /></div>
      ...
      <footer className="py-20 border-t border-white/5 text-center bg-card">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
           <div className="flex items-center gap-3">
              <div className="relative h-8 w-8 rounded-lg overflow-hidden ring-1 ring-white/10"><Image src={RENTALFLOW_LOGO_URL} alt="Logo" fill className="object-cover" unoptimized /></div>
              <span className="font-headline font-bold text-lg tracking-tighter text-foreground">RentalFlow</span>
           </div>
           <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.5em] font-headline opacity-40">© 2026 RENTALFLOW OPERATIONS. ENCRYPTED & PROTECTED.</p>
