          ...
          requests.slice().sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)).map((request) => (
            <Card key={request.id} className="border-none shadow-sm overflow-hidden bg-white rounded-2xl group border border-transparent hover:border-primary/5 transition-all">
              ...
              <CardFooter className="bg-muted/5 p-4 flex flex-col sm:flex-row gap-3 border-t">
                <Button className="w-full sm:flex-1 bg-white hover:bg-primary/5 text-primary rounded-xl font-bold h-12 border border-primary/20 shadow-sm" onClick={() => handleTriage(request)} disabled={isTriaging === request.id}>
                ...
                <Dialog open={isLoggingCost === request.id} onOpenChange={(open) => !open && setIsLoggingCost(null)}>
                  <Button variant="outline" className="w-full sm:flex-1 rounded-xl font-bold h-12 border-primary/20 bg-white shadow-sm" onClick={() => setIsLoggingCost(request.id)}>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="w-full sm:flex-1 rounded-xl font-bold h-12 border-primary/20 bg-white shadow-sm">Update Status</Button>
                  </DropdownMenuTrigger>
